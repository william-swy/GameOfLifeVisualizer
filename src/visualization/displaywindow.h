...
private slots:
  ...
  /**
   * Advances the current gameBoard state forward one generation. Will update all cells with new
   * data. Will update status bar with new generation number.
   */
  void stepForward();

  /*
   * Repeatedly advances the current gameBoard state forward one generation until the user clicks
   * stop. REQUIRES that the game cannot be currently advancing (stopped) to call this function
   */
  void run();
...
signals:
  ...
  /**
   * A signal to notify all Cells that the board state has changed and they should update themselves
   * if necessary.
...
private:
  ...
  /**
   * Checks if gameBoard running speed manipulation buttons can be clicked. If so, they are set to
   * enabled. If not they are set to disabled.
   */
